...
def send_premium_telegram_alert(signal_data):
    ...
    msg = (
        ...
        f"                             @app.route('/api/update-index', methods=['POST'])
def update_index():
    """Updates the globally selected index for the background engine."""
    idx = request.json.get('index', 'NIFTY')
    state = SystemState.query.first()
    if not state:
        state = SystemState(selected_index=idx)
...
def gvn_signal_engine():
    """Background thread with Database-backed Anti-Spam Logic."""
    print("🚀 [GVN SIGNAL ENGINE] Monitoring Alpha Grid with DB Sync...")
    while True:
        try:
            with app.app_context():
                # 1. Check selected index
                state = SystemState.query.first()
                target_idx = state.selected_index if state else "NIFTY"
                
                scanner_data = shared_data.gvn_scanner_data.get(target_idx, [])
                for item in scanner_data:
                    if item.get('levels') and item['ltp'] > item['levels']['i5']:
                        # 2. Check DB if signal already sent in last 15 mins
                        fifteen_mins_ago = datetime.utcnow() - timedelta(minutes=15)
                        existing = TradeSignal.query.filter(
                            TradeSignal.strike == item['strike'],
                            TradeSignal.timestamp > fifteen_mins_ago
                        ).first()
                        ...
                        if not existing:
                            ...
                            # 3. Record in DB
                            new_sig = TradeSignal(strike=item['strike'], signal_type="BUY", price=item['ltp'])
